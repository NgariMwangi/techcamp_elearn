from flask import Flask,render_template,redirect,url_for,request, jsonify,json,session, send_from_directory
...
def courseTopics(title, id):
    journey = requests.get(f'{base_url_journeys}/journeys/{id}')
    all_Sections = requests.get(f'{base_url_journeys}/journeys/{id}/sections')
    topics = CategoryTopics.query.all()
    catIds = []

    for each in CategoryTopic:
        catIds.append(each.categoryid)

    for cat in all_categories:
        if cat.id in catIds:
            print('Yes', cat.id)
            # print()
            for t in CategoryTopic:
                # print(t.id)
        ...
        #     print('No'/)
    print(catIds)
    return render_template('courseTopics.html', journey=journey, all_Sections=all_Sections, CategoryTopic = CategoryTopic, catIds = catIds)
